...
### 시드를 한 번 넣은 로컬 DB 에는 새 마이그레이션이 안 들어간다

`local` 프로필 시드가 `V900` 대라 **적용 이력의 최고 버전이 900** 이 되고, 그 뒤 낮은 번호를 더하면 Flyway 가 순서를 어긴 것으로 보고 기동을 막는다 — `Detected resolved migration not applied to database: N`.
`out-of-order` 를 안 켠다 — 켜면 진짜 순서 사고도 같이 통과한다. **빈 데이터베이스에 처음부터 올린다**(`/verify` 표의 「마이그레이션을 더했으면」 줄).
