...
### `CsrfCookieTest` 가 「쿠키가 없다」로 한 번 깨졌고 재현이 안 됐다

`gradlew build` 한 번에서 `get_issues_cookie_and_header_from_it_passes` 가 실패했고,
같은 코드로 **일곱 번을 더 돌렸는데 안 나왔다**(단독 1회, `integrationTest` 4회, `build` 2회).
원인을 못 짚었으므로 **안 고쳤다** — 대신 그 테스트가 상태 코드를 먼저 보게 했다.

**그 전에는 응답이 어떤 이유로 틀어져도 증상이 「쿠키가 없다」 하나였다.** 다시 나오면
상태 코드가 먼저 말해 주고, 그때 이 절에 무엇이었는지 적는다.
