...
| 2026-09-19 | `I5-1` — **열아홉을 열었다.** 고친 셋: OWASP Top 10 2025 가 308 로 옮겨졌고(`top10.owasp.org/2025`), W3C Trace Context 는 그 URL 이 Recommendation(2021-11-23)이라 「Level 2 초안」 문구를 고쳤고, Testcontainers 이슈 둘은 닫혔다(`11212` → PR `11216`, `11235` 는 중복). **적힌 그대로인 열여섯**: RFC 9110(§13.1.2 `If-None-Match`·§15.5.2 401 의 `WWW-Authenticate` 필수), RFC 9457(Standards Track, 7807 을 폐기, `status` 는 advisory), RFC 4151(`tag:엔티티,날짜:구체부` — 연도만 써도 된다), RFC 5321 §4.5.3.1.3(경로 256 옥텟), RFC 9562(v4 는 version `0b0100`·variant `0b10`), PostgreSQL 17 §13.2.1(「`WHERE` 를 다시 평가한다」), Stripe 멱등(24시간·클라이언트가 만든다·V4 UUID 권장·다른 본문은 오류), Stripe Connect 마켓플레이스(고객이 플랫폼에 내고 플랫폼이 수수료를 떼고 지급), CloudEvents(Graduated 2024-01-25), NIST Rev 4(15자·블록리스트 `SHALL`), Zalando(「RESTful API and Event Guidelines」), SQL Style Guide(CC BY-SA 4.0, `id` 금지), SRE Book(SLI·SLO·SLA), Transactional Outbox, Practical Test Pyramid(Ham Vocke), Money 패턴, Spring Boot Testcontainers(`@ServiceConnection`), Spring Security(현행 7.1.1 — Boot 4.1.1 과 맞다). **안 연 것**: Spring 둘(testing 색인·tracing)·Testcontainers 홈·Stripe Connect 셋·법제처·DDD PDF |
