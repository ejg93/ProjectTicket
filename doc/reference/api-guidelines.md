...
| `GET /api/policies/{code}` | 공개하는 방침 문서의 지금 판 — `code`·`title`·`version`·`effective_at`·`body`(마크다운). 지금은 `privacy_policy`(제30조) 하나. 동의 항목과 표가 다르다 — 동의를 안 받는다 | 공개 | 39-3a |
...
| 400 | 요청을 못 읽었거나 **입력칸 하나를 고치면 되는 위반**이다 — 형식이든 규칙이든(`validation-failed` + `errors[]`) | 깨진 JSON, `@Size` 위반, **모르는 열거값**, 등급 코드 중복 |
...
| 422 | 형식은 맞는데 **입력칸 하나로 못 푸는** 규칙 위반 | 필수 동의 빠짐, 4매 초과 |
...
**400 과 422 의 경계**: 「사용자가 **입력칸 하나를 고치면 되나**」로 가른다(`45d-c`). 되면 400 `validation-failed` 에 `errors[{field, message}]` — 화면이 그 칸을 짚어 말한다(비밀번호가 이메일을 담음 · 정렬 허용표 · 등급 코드 중복 · 판매 창). 안 되면 422 이고 `errors[]` 대신 값을 싣는다 — 필수 동의 빠짐은 빠진 칸이 요청에 없고, 4매 초과는 개수라는 요청 전체의 성질이며, `seat-not-in-performance`(`seat_ids`)·`unknown-consent-item` 은 화면이 낡은 목록을 보낸 것이라 칸을 고쳐 풀 일이 아니다.
...
| `consent-item-not-found` | 404 | 그런 코드의 동의 항목이 없다(약관·수집 이용 안내 화면) | | 39-1 |
| `policy-not-found` | 404 | 그런 코드의 방침 문서가 없다(처리방침 화면) | | 39-3a |
