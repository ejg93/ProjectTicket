...
| 400 | 요청을 못 읽었거나 **칸 하나를 짚을 수 있는 위반**이다 — 형식이든 규칙이든(`validation-failed` + `errors[]`) | 깨진 JSON, `@Size` 위반, **모르는 열거값**, 등급 코드 중복 |
...
| 422 | 형식은 맞는데 **칸 하나로 못 짚는** 규칙 위반 | 필수 동의 빠짐, 4매 초과 |
...
**400 과 422 의 경계**: 「칸 하나를 짚을 수 있나」로 가른다(`45d-c`). 짚을 수 있으면 400 `validation-failed` 에 `errors[{field, message}]` — 화면이 그 칸 옆에서 말한다(비밀번호가 이메일을 담음 · 정렬 허용표 · 등급 코드 중복 · 판매 창). 못 짚으면 422 — 필수 동의 빠짐은 빠진 칸이 요청에 없고, 4매 초과는 좌석 칸 하나가 아니라 개수라는 요청 전체의 성질이다.
