...
| `SqlTextTest` 결합 | 4 테스트 | 〃 | `.sql(` 인자가 문자열 리터럴 하나가 아닌 것, 리터럴 안의 보간이 허용 목록(`${order.sql}`·대문자 `const val`) 밖인 것(`G10`, `D9`). CodeQL 이 도우미가 `StringBuilder` 로 조립해 넘긴 SQL 을 못 잡아서 세웠다(`46c` 탐침). **`JdbcClient` 밖의 SQL 입구는 못 본다** | `G10` 2026-09-26 — 도우미의 `StringBuilder` 조립·원시 문자열 안 `${name}` 을 넣으니 둘 다 빨강. 탐침 `sql-text-builder`·`sql-text-interpolation` |
