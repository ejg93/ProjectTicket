...
| `ScreenLengthTest` | 4 테스트 | `gradlew test`(빠른 레인) | 화면 폼의 `maxLength` 와 요청 `@Size(max)` 가 갈리는 것(`G9`), **하한도** — `@Size(min)` 이 0 보다 크면 `minLength`(`G9-1`, `@Password` 15) — 서버가 400 으로 돌려보낼 글자를 화면이 받아 준다. 서버는 리플렉션(메타 애너테이션까지), 화면은 `*-form.tsx` 글자라 그 파일들이 `inputs.files`·backend 지문에 있다 | `G9` 2026-09-26 — 탐침 `screen-length`(가입 이름 50→60) 빨강 「칸에 `maxLength={50}` 가 없다」. `G9-1` 2026-09-26 — 탐침 `screen-min-length`(가입 비밀번호 15→12) 빨강 |
...
| `SqlTextTest` | 4 테스트 | 〃 | `where` 에 좌석 `status` 조건이 없는 `update performance_seat`(`G7b`, `D4`). 흐름 시험은 한 사람씩 돌아서 덮어쓰기를 못 본다. 원시 문자열과 `sql(` 리터럴을 읽는다 — **조립한 SQL 은 못 읽고, 조립은 아래 행이 막는다** | `G7b` 2026-09-25 — 조건 없는 좌석 UPDATE 를 넣으니 빨강. 탐침 `sql-seat-update` |
| `SqlTextTest` 결합 | 4 테스트 | 〃 | `.sql(` 인자가 문자열 리터럴 하나가 아닌 것, 리터럴 안의 보간이 허용 목록(`${order.sql}`·같은 파일의 `const val`) 밖인 것(`G10`, `D9`). 원시 문자열은 이스케이프 없이, `with(jdbc) { sql(…) }` 꼴까지 읽는다. CodeQL 이 도우미가 `StringBuilder` 로 조립해 넘긴 SQL 을 못 잡아서 세웠다(`46c` 탐침). **`JdbcClient` 밖의 SQL 입구는 못 본다** | `G10` 2026-09-26 — 도우미의 `StringBuilder` 조립·원시 문자열 안 `${name}` 을 넣으니 둘 다 빨강. 탐침 `sql-text-builder`·`sql-text-interpolation` |
...
| `TestConventionTest` | 4 테스트 | 〃 | 레인·태그·바탕의 약속이 갈리는 것(`G7d`, `D8`) — 바탕 없이 DB·`MockMvc` 를 쓰는 테스트(빠른 레인에서 컨테이너 없이 돈다), 사유 없이 손으로 단 `@Tag("db")`, 「레인」 표와 `build.gradle.kts` 의 태스크·태그, 커밋 레인 자식이 바탕 `PREFIX` 를 가리는 것(`G11` — 정리가 그 행을 못 지워 재사용 컨테이너에 남는다) | `G7d` 2026-09-25 — 셋 각자 위반을 넣으니 셋 다 빨강. 탐침 `test-without-base`·`test-hand-tag`·`test-lane-name`. `G11` 2026-09-26 — 자식에 `PREFIX` 를 다시 선언하니 빨강. 탐침 `test-prefix-shadow` |
