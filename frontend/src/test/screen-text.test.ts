...
describe("화면 문구는 존댓말이다", () => {
  ...
  it("훑을 화면이 있다", () => {
    ...
    // 2026-09-19 실측으로 화면이 스물이다. 화면이 줄어드는 일은 없으니 절반을 바닥으로 둔다(마무리 9차가 다시 쟀다).
    expect(files.length).toBeGreaterThan(10);
  });

  it("걷은 문구가 실제 화면 수만큼 있다", () => {
    // AST 를 걷는 방식이라 조용히 아무것도 못 걷는 고장이 난다. 2026-09-19 실측 113 의 절반이 바닥이다.
    expect(phrases.length).toBeGreaterThan(56);
  });
