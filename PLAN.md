...
| 19 | 동시성 계측 | 완료 — `LockComparisonTest`(`measure` 태그, 스크래치 표, 100·1000 스레드 × 셋), gradle `measure` 레인(`build` 밖, 늘 다시 돈다), `doc/notes/lock-comparison.md`(두 실행 + 기계 사양), **ADR 0006**: 1000 스레드에서 조건부 1 : 낙관 1.3 : 비관 4.2, 셋 다 승자 하나 — 조건부 UPDATE 유지, `version` 컬럼 안 둔다. `D4` 낙관락 행 닫음, ADR 0003 포인터 | 완료 |
