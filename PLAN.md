...
| 17b | 환불 되살리기 스윕 | `requested` 로 남은 환불(PG 환불 뒤 ③ 전에 죽은 것)을 같은 키(환불 id)로 PG 에 다시 보내 `done` 으로. **축**: `D4`(재시도 — PG 가 같은 키에 같은 답을 준다). **강제 지점**: 조건부 UPDATE + 테스트. **건드리는 자리**: `payment/RefundSweeper`, `HoldSweeper` 와 같은 스케줄러. **닫힘**: `RefundSweeperTest` | 17 |
