...
| 17a | 회차 취소 | 기획사가 `open` 회차를 `cancelled` 로. 모든 예매 `cancelled`, `reserved` 는 **전액 환불**(구간 무관), 알림 이벤트 일괄(아웃박스). check·전이 트리거에 `cancelled` 추가. **축**: `D3`·`D6`. **강제 지점**: 한 트랜잭션 + 아웃박스 원자성 테스트. **건드리는 자리**: 새 `V`, `event/PerformanceCancelService`, `payment/Refund*`. **닫힘**: `PerformanceCancelTest.every_reservation_refunded_in_full` + `SettlementAggregateTest.cancelled_performance_settles_to_zero`(27 이 전이가 없어 못 세운 것) | 17·25·27 |
| 17b | 환불 되살리기 스윕 | 둘 — ⓐ `requested` 로 남은 환불(PG 환불 뒤 ③ 전에 죽은 것)을 같은 키(환불 id)로 PG 에 다시 보내 `done` 으로. ⓑ `payment.late` 감사만 남은 승인 결제(예매가 `reserved` 가 아닌 `approved` 결제)에 전액 환불 행(`reason = payment_late`, 율 0)을 만든다 — 16 이 PG 취소만 보내고 행은 안 만들었다. **축**: `D4`(재시도 — PG 가 같은 키에 같은 답을 준다). **강제 지점**: 조건부 UPDATE + 테스트. **건드리는 자리**: `payment/RefundSweeper`, `HoldSweeper` 와 같은 스케줄러. **닫힘**: `RefundSweeperTest` | 17 |
