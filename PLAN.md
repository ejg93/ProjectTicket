...
| 4 | 감사·동의 포팅 | `audit`(누가 무엇을 언제)·`consent`(약관 동의 이력) Kotlin 포팅. `SignupService` 트랜잭션에 동의·감사 기록을 붙인다(청크 3 이 자리를 비워 뒀다). **축**: `D9`·개인정보 최소수집. **강제 지점**: 트리거(감사 행 수정 금지). **건드리는 자리**: 신설 `audit/`·`consent/`, `V3`, `SignupService`·`AuthController` 수정. **닫힘**: `AuditImmutabilityTest` | 3 |
| 5 | 관측 포팅 | `observability`(요청 로그 필터·추적 ID·MDC)·actuator. `ProblemFactory` 에 `trace_id` 를 붙인다(청크 3 이 비워 뒀다). **축**: `D10`. **강제 지점**: 테스트(추적 ID 가 요청마다 다르다, 오류 본문의 `trace_id` 가 로그와 같다). **건드리는 자리**: 신설 `observability/`, `ProblemFactory` 수정. **닫힘**: `TraceIdFilterTest` | 3 |
