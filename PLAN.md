...
| 34 | 좌석 분산락 비교 **[ADR 0008 은 Fable]** | 조건부 UPDATE vs **Redis 분산락**(`SchedulerLock` 과 같은 `SET NX`)을 33 환경에서 재고 ADR 0008. **33 이 Redisson 을 안 들이기로 해서**(사용자 선택) 비교 대상이 바뀌었다 — 재는 질문은 그대로다: 좌석 하나를 두고 다툴 때 DB 조건부 UPDATE 와 Redis 락 중 무엇이 싼가. **축**: 측정. **강제 지점**: 없다. **건드리는 자리**: `reservation/`, `doc/adr/0005`. **닫힘**: ADR 에 두 수치 | 33 |
