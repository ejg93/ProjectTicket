...
| 40c | 회차 하나 조회 | **마무리 9차가 찾았다**: `D5` 표가 `GET /api/performances/{id}` 를 `40` 몫으로 적어 뒀는데 `40a` 가 안 만들었고, **회차 등록 201 의 `Location` 이 그 주소를 가리켜 404 다**(RFC 9110 은 `Location` 이 만들어진 자원을 가리키라고 한다). **축**: 표준(RFC 9110) + `D5`. **강제 지점**: 테스트 — 등록 201 의 `Location` 을 그대로 GET 해서 200 인가. **건드리는 자리**: `event/PerformanceQuery.kt`(신설)·`EventController` 또는 신설 컨트롤러·`SecurityConfig`. **닫힘**: 그 테스트 초록 | 40a |
...
| 41-1 | 화면과 서버의 상한을 묶는다 | **마무리 9차 독립 리뷰가 찾았다**: `SeatMap.MAX_SEATS`(4)와 `SeatHoldService.MAX_SEATS_PER_HOLD` 가 **두 벌인데 대조가 없다.** 같은 묶음의 `I4-2` 가 앱↔DB 의 같은 모양을 테스트로 묶었는데 화면↔서버는 안 묶였다 — 갈리면 화면이 5석을 보내고 422 `over-limit` 이 난다(사용자는 고를 수 있던 것을 못 산다). **축**: `D14` 강제 지점. **강제 지점**: 테스트 — 한쪽이 Kotlin 이고 한쪽이 TS 라 **파일을 글자로 읽어 대조한다**(`StackVersionConsistencyTest` 가 같은 모양이다). 어느 쪽에 둘지(백엔드 테스트가 `.tsx` 를 읽나, 화면 테스트가 `.kt` 를 읽나)가 이 청크의 결정이다. **건드리는 자리**: 그 테스트 하나. **닫힘**: 한쪽 수를 바꾸면 빨개진다 | 42 |
...
| 46c | CodeQL `JdbcClient` 싱크 | **`P10` 이 정했다**: 첫 측정 0건은 SQL 에서 「없다」가 아니라 「안 봤다」다 — 기본 질의가 `JdbcClient.sql()` 을 싱크로 모른다(ProjectShop `2e-4` 가 같은 실행에서 `JdbcTemplate` 은 잡고 `JdbcClient` 는 안 잡는 것을 쟀다). 이 저장소의 데이터 접근이 전부 `JdbcClient` 고 `EventQuery` 가 `order by` 를 문자열로 붙인다. **축**: `D18`·`D9`. **강제 지점**: CI — 데이터 확장(`sinkModel`)을 `codeql.yml` 의 `config.packs` 로 든다. **건드리는 자리**: 신설 `.github/codeql/extensions/projectticket-java/`(ProjectShop 의 둘을 옮긴다 — `qlpack.yml`·`models/jdbc-client.model.yml`)·`codeql.yml`. **닫힘**: 탐침 가지(`probe/codeql`)에 요청 파라미터를 그대로 `sql()` 에 붙인 커밋을 밀어 `java/sql-injection` 이 뜨는 것을 보고 가지를 지운다 — 그 뒤 첫 초록의 경보 수를 `D18` 문턱 표에 적는다 | P10 |
