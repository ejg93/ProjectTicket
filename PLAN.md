...
| P10 **[Fable]** | `quality-gates.md` 머리말·문턱 | `P10a` 가 게이트 표를 실물로 다시 썼고 **이식 머리말과 문턱이 남았다**(점검 6차 — 겹치던 자리를 여기로 좁혔다). **축**: 규약. **강제 지점**: CI 잡 목록이 문서와 같다. **CodeQL 확장 팩**(`JdbcClient` 싱크)을 들일지도 여기서 정한다 — 첫 측정값이 근거다. **건드리는 자리**: 그 문서. **닫힘**: 머리말이 없고 문턱 행이 첫 측정값을 든다(`46a` 가 CodeQL 행을 세웠다) | 46a·47 |
