...
| 14 | 선점 만료 스윕 | 완료 — `HoldSweeper.sweep()` 30초(`SchedulingConfig` 가 스위치). 예매 `held→expired`(`held_until < now()`) 뒤 그 예매 id 로 좌석 `held→available`. 둘 다 조건부라 멱등, `paying` 은 안 걸린다. 만료마다 `reservation.expired` 감사. 닫힘: `HoldSweeperTest`(넷 — 만료 해제·`does_not_release_confirmed`·살아있는 것·멱등) | 완료 |
