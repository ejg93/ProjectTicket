...
| 23a | 관문과 멱등 재시도 | 성공한 선점이 토큰을 반납해서(23), **응답을 못 받은 클라이언트가 같은 멱등키로 재시도하면 관문이 429 로 끊는다** — `D4` 「멱등키」가 약속한 「저장된 본문을 그대로 돌려준다」가 관문 뒤에 있어 안 닿는다(마무리 5차 독립 리뷰가 잡았다). 데이터는 안 깨진다(줄에 다시 서면 복구된다) — 계약이 어긋난 것이다. 고를 것 둘: ⓐ 반납할 때 **활성 집합에서만 빼고 토큰은 짧은 TTL 로 남긴다**(정원은 바로 돌고 재시도는 지나간다) ⓑ 관문이 멱등키를 보고 이미 처리된 키면 통과시킨다. **축**: `D4`·`D12`. **강제 지점**: 테스트(같은 키 재시도가 저장된 201 을 받는다). **건드리는 자리**: `queue/QueueGateFilter`·`AdmissionService`, `D12` 「토큰」. **닫힘**: `QueueGateTest.a_retry_with_the_same_key_replays` | 23 |
...
| 35 | 부하 2차 | 33 환경에서 31 재실행, `D13` 대조. **축**: `D13`. **강제 지점**: 없다. **건드리는 자리**: `doc/notes/load-2.md`. **닫힘**: 목표 대비 표 — 완료(`load-2.md`. **대조 전에 구성이 말과 달랐다** — nginx 가 `upstream` 의 이름을 기동 때 한 번만 풀어 `--scale app=3` 이어도 **한 대가 전부 받았다**(9,174 : 0 : 0). `resolver` + 변수 `proxy_pass` 로 고쳐 2,675 : 2,632 : 2,651. 33 의 healthcheck 도 같이 고쳤다(JRE 에 `wget` 이 없다). 진짜 셋의 값은 한 대보다 나쁘다 — 선점 p95 621ms·HTTP p95 1.21s·424 rps 로 `D13` 셋 중 셋을 못 지켰다. 기계가 하나라서다. **목표를 안 느슨하게 하고 모양별로 갈랐다** — `D13` 에 「한 대」·「셋(같은 기계)」 두 열. 5xx 0, 대기열 대기는 등식의 1/6. 스냅샷 비율은 스크립트가 폴링을 안 해 미측정 → `44`) | 완료 |
