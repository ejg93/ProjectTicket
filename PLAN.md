...
| 13a | 예매 교차 표 제약 | 점검 1차가 찾은 누락 둘을 트리거로 내린다 — ⓐ `payment.amount = reservation.total_amount`(결제 행 삽입 때, `D14` 「합계를 저장하는 순간」) ⓑ `reservation_seat.performance_seat_id` 의 회차 = 예매의 회차(다른 회차 좌석이 기록에 못 든다). 둘 다 다른 표를 봐야 해서 check 로 못 걸고 즉시 트리거다. **축**: `D4`·`D14` 「불변식」. **강제 지점**: 트리거 + 테스트. **건드리는 자리**: `V12`, 기존 테스트 둘에 케이스. **닫힘**: `ReservationSeatConsistencyTest`·`PaymentConfirmTest` 에 위반 케이스 | 18 |
