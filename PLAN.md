...
| 13b | 멱등키 청소 | `D4` 가 「보관 24시간」이라 정했고 `V7` 주석과 `idempotency_key_created_idx` 가 그것을 전제하는데 **지우는 코드가 없다**(점검 2차). 스케줄러 열 중 아무도 안 본다 — 표가 무한히 는다. `OutboxCleaner`(25a)와 같은 모양이다. **축**: `D4` 「멱등키」 + `D2` 수명. **강제 지점**: 테스트(24시간 지난 행은 지우고 경계는 남긴다) + 33 의 스케줄러 락. **건드리는 자리**: 신설 `idempotency/IdempotencyCleaner`. **닫힘**: `IdempotencyCleanerTest` | 13 |
| I2-1 | 열거형과 `check` 대조 | DB 가 값 목록을 닫아 둔 자리 **14** 곳과 Kotlin 열거형 **10** 개가 갈리는 것을 막는 것이 없다(점검 2차). 값을 열거형에만 더하면 삽입이 런타임에 제약으로 죽고, `check` 에만 더하면 열거형 변환이 죽는다 — 둘 다 컴파일은 초록이다. ProjectShop 의 `EnumConstraintTest` 가 하던 일이고 `D18` 은 이 저장소에도 있는 것처럼 적어 뒀다. **축**: `D14` 「열거값을 어디에 두나」 + `D18`. **강제 지점**: 테스트(마이그레이션의 `check (x in (...))` 와 열거형 `code` 집합이 같다). **건드리는 자리**: 신설 `EnumConstraintTest`. **닫힘**: 그 테스트 | I2 |
| P10a | `D18` 을 이 저장소 것으로 | 게이트 표가 ProjectShop 것이다 — 아홉 중 `ArchitectureTest`·`StackVersionConsistencyTest` 둘만 실재하고, 나머지 일곱(`EnumConstraintTest`·`OpenApiSpecTest`·`ActionPermissionTest`·`SellerSchemaTest`·`PublicSurfaceTest`·`TestConventionTest`·`PlanProgressConsistencyTest`)은 없다. 테스트 수도 972+29 로 적혀 있는데 실제 310 이고 frontend 는 아직 없다. **문서의 목적이 「무엇이 막는다고 말할 수 있나」라 표가 틀리면 반대로 일한다.** `47` 이 detekt 행만 더하고 나머지를 안 봤다. **축**: `D18` 자신 + 관례. **강제 지점**: 문서 — 더 아래로 못 내린다. 다만 `I2-1` 이 세우는 게이트는 여기 행이 된다. **건드리는 자리**: `doc/reference/quality-gates.md`(이식 머리말 삭제 포함). **닫힘**: 표의 모든 행이 실재하는 게이트다 | I2 |
