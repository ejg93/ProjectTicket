 ...
 *
 * **스윕과 빈이 갈린 이유는 자기 호출이다** — 같은 클래스 안에서 부르면 프록시를 안 지나 `@Transactional` 이 통째로 무시된다(`stack.md`).
 *
 * **`REQUIRES_NEW` 다**(사용자 선택). 28 전에는 릴레이가 자기 트랜잭션 안에서 Spring 이벤트를 발행해서, 이 경계가 없으면
 * 알림 쪽 실패가 릴레이 트랜잭션을 롤백시켜 `published_at` 이 안 찍히고 같은 사건이 1초마다 영영 재발행됐다.
 * **28 이 그 사슬을 끊었다** — 소비는 이제 브로커 너머 다른 스레드·다른 트랜잭션이라 릴레이를 못 건드린다.
 * 경계를 남겨 두는 이유는 스윕이 이 표를 여러 트랜잭션으로 나눠 만지기 때문이고, `D11` 의 「소비자 하나의 결함이 발행자를 멈추지 않는다」는
 * 이제 프로세스 경계가 지킨다.
 *
