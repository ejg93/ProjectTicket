 ...
 *
 * `ExceptionTranslationFilter` 뒤에 있어서 예외를 던져 진입점으로 보낼 수 없다. 그래서 진입점을 직접 부른다 —
 * 401 은 RFC 9110 §15.5.2 대로 `WWW-Authenticate` 를 달아야 하고 본문은 RFC 9457 이어야 하는데, 상태 코드만 쓰면 둘 다 빠진다.
 *
 ...
 */
class AccountLivenessFilter(
    private val jdbc: JdbcClient,
    private val entryPoint: ProblemEntryPoint,
    ...
    override fun doFilterInternal(request: HttpServletRequest, response: HttpServletResponse, chain: FilterChain) {
        ...
        if (principal is TicketUser && !isAlive(principal.id)) {
            expire(request)
            entryPoint.commence(request, response, InsufficientAuthenticationException("계정이 살아 있지 않다"))
            return
        }
        chain.doFilter(request, response)
    }

    /** 상태 리터럴을 SQL 에 안 박는다 — [AccountStatus] 가 유일한 출처다 */
    private fun isAlive(accountId: Long): Boolean =
        jdbc.sql("select exists(select 1 from account where account_id = :id and deleted_at is null and status = :active)")
            .param("id", accountId)
            .param("active", AccountStatus.ACTIVE.code)
